import os
import numpy as np
import torch  # <- Esto faltaba para cargar los point clouds como tensors
...
class ObjectFolder2Dataset(Dataset):
    ...
    def _load_samples(self):
        samples = []
        pc_dir = os.path.join(self.root_dir, "pointclouds")
        img_dir = os.path.join(self.root_dir, "RGB_images")
            ...
            if pc_file.endswith(".npy"):
                ...
                if os.path.exists(img_path):
                    if self.return_label:
                        # Dummy label: category inferred from ID prefix (modify this logic as needed)
                        class_name = obj_id  # Replace with actual logic if needed
                        samples.append((
